...
class MpHtmlParser {
  ...
  // 设置属性
  setAttr() {
    const name = this.getName(this.attrName);
  ...
  matchAttr(node) {
    ...
    switch (node.name) {
      ...
      case 'font':
        ...
        if (attrs.size) {
          ...
          const map = ['xx-small', 'x-small', 'small', 'medium', 'large', 'x-large', 'xx-large'];
    ...
    // 压缩 style
    const styles = style.replace(/&quot;/g, '"').replace(/&amp;/g, '&').split(';');
    style = '';
    for (let i = 0, len = styles.length; i < len; i++) {
      const info = styles[i].split(':');
    ...
    if (node.name == 'img' || node.name == 'picture') {
      ...
      const check = item => styleObj[item] && !styleObj[item].includes('auto');
      if (check('width')) {
        const parent = this.parent();
  ...
  EndTag(c) {
    if (blankChar[c] || c == '>' || c == '/') {
      const name = this.getName(this.section());
