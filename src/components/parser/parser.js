  ...
  created() {
    ...
    this.imgList.setItem = function(i, src) {
      ...
      if (src.includes('data:image')) {
        const info = src.match(/data:image\/(\S+?);(\S+?),(.+)/);
        if (!info) return;
        const filePath = `${wx.env.USER_DATA_PATH}/${Date.now()}.${info[1]}`;
        fs && fs.writeFile({
          filePath,
          data: info[3],
          encoding: info[2],
  ...
  methods: {
    ...
    getText(ns = this.data.html) {
      ...
      for (var i = 0, n; n = ns[i++];) {
        if (n.type == 'text') txt += n.text.replace(/&nbsp;/g, '\u00A0').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
          ...
          // 块级标签前后加换行
          const br = n.name == 'p' || n.name == 'div' || n.name == 'tr' || n.name == 'li' || (n.name[0] == 'h' && n.name[1] > '0' && n.name[1] < '7');
    ...
    setContent(html, append, _watch) {
      ...
      if (!html) {
        ...
        const parser = new Parser(html, this.data);
        // 缓存读取
        if (this.data.useCache) {
          const hashVal = hash(html);
      ...
      const ns = this.selectAllComponents('.top,.top>>>._node');
    ...
    _tap(e) {
      if (this.data.gestureZoom && e.timeStamp - this._lastT < 300) {
        const initY = e.detail.y - e.currentTarget.offsetTop;
        if (this._zoom) {
          ...
          const initX = e.detail.x - e.currentTarget.offsetLeft;
          this._initY = initY;
    ...
    _touchmove(e) {
      const diff = e.touches[0].pageX - this._lastX;
